...
public abstract class UniqueIdAbstract implements IUniqueId {
    ...
    public String getUniqueId(Context context) {
        return Build.BOARD + Build.BRAND + Build.DEVICE + Build.DISPLAY
                ...
                + Build.PRODUCT + Build.TAGS + Build.TYPE + Build.USER + Build.CPU_ABI;
    }
     ...
     * 获取指定的字符串的MD5
     */
    protected String calcMD5(String originString) {
        try {
            //创建具有MD5算法的信息摘要
     ...
     */
    protected String byteArrayToHex(byte[] b) {
    ...
    protected boolean checkReadPrivilegedPhoneStatePermission(Context context){
        if (Build.VERSION.SDK_INT >= 29) {
            if (context.checkSelfPermission("android.permission.READ_PRIVILEGED_PHONE_STATE") == PackageManager.PERMISSION_DENIED) {
                return false;
            }
        }
        return true;
